        ...
        return {
            ...
            create : function(loginState, data){
                var temp = document.implementation.createDocument('http://www.w3.org/1999/xhtml', 'div', null),
                    ...
                    recommendPopup = Extension.UI.createElement('span', { //shizato
                            'id' : 'wix-recommend-popup'
                        })
...
Wix.main = new WIXClass({
    ...
    view : {
        login : function(){
            ...
            //レコメンドボタンの表示 shinzato
            var recommendButton;
            recommendButton = Extension.UI.createElement("button", {
              'class' : 'wix-recommend'
            }, 'RECOMMEND');
            array.push(recommendButton);
            ...
            for (var key in defaultBookmarks) {
              ...
              var default_key;
                if(defaultBookmarks.hasOwnProperty(key)){
                  ...
                  /*chrome.runtime.sendMessage({method: 'getItem', key: default_id}, function (response) {
                    if (response.data) {
                      default_key = response.data;
                    }
                  });*/
                  default_key = localStorage[default_id];
            ...
            //カスタムボタン
            /*var customButton = Extension.UI.createElement("button", {
              'class' : 'wix-custom'
            }, 'custom');
            array.push(customButton);*/
